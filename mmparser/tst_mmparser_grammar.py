...
"""tst_mmparser_grammar.py - runs the unit tests for the mmparser-grammar
"""

import os
import sys

sys.path.insert(0, os.path.join('..', 'DHParser'))
...
if __name__ == '__main__':
    ...
    # set_preset_value('syntax_variant', 'regex-like')
    finalize_presets()

    if args.scripts:
        dsl.create_scripts(os.path.join(scriptdir, 'mmparser.ebnf'))
    ...
    if arg.endswith('.ebnf'):
        ...
        recompile_grammar(os.path.join(scriptdir, 'mmparser.ebnf'),
                          force=False)
        sys.path.append('.')
        from mmparserParser import parsing, ASTTransformation, junctions, targets
