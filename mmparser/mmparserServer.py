...
"""mmparserServer.py - starts a server (if not already running) for the
                    compilation of mmparser
"""
...
class mmparserLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "mmparser-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = mmparserCPUBoundTasks(self.lsp_data)
        self.blocking = mmparserBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from mmparserParser import batch_process
        args = argstr.split(' ')
    ...
    #     from mmparserParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new mmparserServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('mmparser_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    mmparser_lsp = mmparserLanguageServerProtocol()
    lsp_table = mmparser_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', mmparser_lsp.simply_compile)
    mmparser_server = Server(rpc_functions=lsp_table,
                        cpu_bound=mmparser_lsp.cpu_bound.lsp_table.keys(),
                        blocking=mmparser_lsp.blocking.lsp_table.keys(),
                        connection_callback=mmparser_lsp.connect,
                        server_name="mmparserServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        mmparser_server.echo_log = True if port >= 0 and host else False
        msg = mmparser_server.start_logging(log_path.strip('" \''))
        if mmparser_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        mmparser_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            mmparser_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing mmparser-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python mmparserServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python mmparserServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python mmparserServer.py --stream\n'
             + '    python mmparserServer.py --stopserver\n'
             + '    python mmparserServer.py --status\n'
             + '    python mmparserServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python mmparserServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
