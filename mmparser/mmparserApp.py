...
"""mmparserApp.py - a simple GUI for the compilation of mmparser-files"""
...
import mmparserParser


class mmparserApp(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title('mmparser App')
        ...
        if 'html' in mmparserParser.targets or 'HTML' in mmparserParser.targets:
            self.target = 'html'
        elif len(mmparserParser.targets) == 1:
            self.target = list(mmparserParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = mmparserParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not mmparserParser.main(called_from_app=True):
        app = mmparserApp()
        app.mainloop()
