...
As you can see, the "Termgliederung" looks like an upside down tree. In this case it is even a binary tree, but this merely reflects the contingent fact that all relations that we deal with in this trivial example, i.e. `+`, `-`, `*`, `:`, are binary relations. Generally speaking, the syntactical structure of any formula is a tree-structure that consists of "nodes" and "branches" and where every node can have an arbitrary number of "children", a "child" being a node that is directly connected via a branch to its "parent" node. (You can think of a "node" as a point where a tree branches out.)
...
The only downside of S-expression is that you need to keep track of all the opened parentheses. However, modern editors help by highlighting the matching parentheses if you use the Lisp or Scheme mode. In the following, I will mostly use S-expressions for the syntax trees. 
...
If this appears too verbose (after all `(formular (expression (number "2")))` or even `(formualar (number "2")))` might be enough to capture the meaning of the formula), keep in mind that what a parser yields is a concrete syntax-tree that closely reflects the working mechanism of the parser or, if the parser was generated from a formal grammar, the structure of that grammar. You might as an exercise think about a suitable transformation rule to make the syntax-tree more compact (without losing any relevant information!) However, to demonstrate that according to our grammar, an expression may really just consist of a single term which consists of a single factor, this example may suffice. Now, let's parse our original formula "2 + 4 \* 3" again::
...
3. Collective terms or summary concepts which in a grammar typically appear as symbols defined by a sequence of alternatives (e.g. expression, term, factor) can safely (i.e. without loss of information) be replaced in the syntax-tree by the respective concrete alternative as long as the alternatives do not overlap with those of any other collective term. Keep that in mind when adding symbols to the "@hide"-directive.
...
* From the generated arithmetic4Parser module the function compile_snippet is imported. Note that arithmetic4Parser.py does not necessarily need to be called from the command line as before, it can also be imported from another Python module, in an interactive Python session or from a Python script. The function compile_snippet(str) is also generated and does just that: parse and compile a piece of source text and return the result and a (hopefully empty) list of errors. As in our case the compilation consists only of the parsing stage and that returns a syntax-tree, the result is the root-node of that syntax-tree, which is an instance of DHParser's nodetree.Node class.
...
* From the concrete syntax tree an abstract syntax tree (AST) can be derived that expresses the structure of sentences of a formal language in a semantically meaningful way. (Well-written grammars yield concrete syntax trees that already come close to the intended abstract syntax trees.)
...
* Abstract syntax trees can also be used as a staring point to serialize the statements of a formal language in different notations. This also allows to convert between different formal notations for the same domain. For example one could convert arithmetical formulae between the common [infix notation](https://en.wikipedia.org/wiki/Infix_notation) and the [reverse polish notation](https://en.wikipedia.org/wiki/Reverse_Polish_notation).

In the following parts og this tutorial, we will learn how logical formulae can be converted between the Russell-Peano-notation and the modern notation used for logical formulae, today.
