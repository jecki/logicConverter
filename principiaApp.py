...
"""principiaApp.py - a simple GUI for the compilation of principia-files"""
...
class principiaApp(tk.Tk):
    def __init__(self):
        ...
        grammar = principiaParser.parsing.factory()
            ...
            if self.names:
                ...
                if len(self.names) == 1:
                    ...
                    self.worker = threading.Thread(
                        target = principiaParser.batch_process,
    ...
    def compile_single_unit(self, source, target, parser):
        if target == ALL_TARGETS_SPECIAL:
            path = principiaParser.junctions
        else:
            path = connection(principiaParser.junctions, target, "CST")
    ...
    def write_or_update_config_file(self, path, config) -> bool:
        ...
        ts2p_new = 'principia' not in config.sections()
        if ts2p_new:
            config['principia'] = {}
        cfg = get_config_values('principia.*')
        i = len('principia.')
        cfg = {k[i:]: str(v) for k, v in cfg.items()}
        config['principia'].update(cfg)
        try:
    ...
    def on_export_test(self):
        ...
        if ftype == 'config':
            ...
            if ftype == 'test':
                stages = (UNIT_STAGES
                    | frozenset(j.dst for j in principiaParser.junctions))
            ...
            if error_level < ERROR:
                for stage, result in self.all_results.items():
                    ...
                    tests[stage] = cases
            unit = { parser: tests }
            cfg_data = dump_config_data('principia.*', use_headings=False)
...
if __name__ == '__main__':
    ...
    if not principiaParser.main(called_from_app=True):
        app = principiaApp()
        app.mainloop()
